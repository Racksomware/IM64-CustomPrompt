...
func _ready():
	...
	coin_counter.text = "$ x " + str(SOGlobal.save_data.get_total_coin_count())
	red_coin_counter.text = "& x " + str(SOGlobal.save_data.get_total_red_coin_count())
...
func change_menu_selection(desired_selected : int) -> void:
	...
	match selected_menu_option:
		0: resume_label.label_settings.font_color = Color(1, 1, 1)
		1: respawn_label.label_settings.font_color = Color(1, 1, 1)
		2: generate_new_level_label.label_settings.font_color = Color(1, 1, 1)
		3: view_stage_label.label_settings.font_color = Color(1, 1, 1)
		4: history_label.label_settings.font_color = Color(1, 1, 1)
		5: settings_label.label_settings.font_color = Color(0.581, 0.581, 0.581, 1.0)
		6: exit_label.label_settings.font_color = Color(1, 1, 1)
...
func call_selection_function(desired_button : int) -> void:
	match desired_button:
		0: # resume
			_unpause()
		1: # retry
			SOGlobal.current_mario._respawn_mario(preload("res://mario/sfx/enter_painting.WAV"))
			for child in SOGlobal.get_children():
				...
				if child is RedCoin:
					child._respawn()
			SOGlobal.current_level_manager.reset_red_coin_count()
			_unpause()
		...
		3: # view level
			print("todo")
		4: # seed history
			var seed_history_ui = preload("res://mario/other/seed_history.tscn").instantiate()
			seed_history_ui.pause_menu = self
			add_child(seed_history_ui)
			hide_pause_menu = true
		5: # open settings
			print("ToDo! Remaking this shit.")
			# var settings_ui = preload("res://mario/other/settings_menu.tscn").instantiate()
...
@onready var red_coin_counter 	= $Control/StatDisplay/HBoxContainer/RedCoinCounter
